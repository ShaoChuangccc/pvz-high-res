...
const plantIcons = {
    Peashooter: (x, y, scale = 1, alpha = 1) => {
        ...
        ctx.fill();
        ctx.beginPath();
        ctx.ellipse(0, 25, 20, 8, Math.PI / 2, 0, Math.PI * 2);
        ctx.fill();
    ...
    Sunflower: (x, y, scale = 1, alpha = 1) => {
        ...
        ctx.fillRect(-4, 0, 8, 30); // 茎
        for (let i = 0; i < 10; i++) { // 花瓣
            ctx.fillStyle = '#fbc02d';
            ctx.beginPath();
            ctx.ellipse(Math.cos(i * Math.PI / 5) * 18, Math.sin(i * Math.PI / 5) * 18, 14, 6, i * Math.PI / 5, 0, Math.PI * 2);
            ctx.fill();
        }
    ...
    Wallnut: (x, y, scale = 1, alpha = 1) => {
        ...
        ctx.beginPath();
        ctx.ellipse(0, 5, 24, 30, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#000';
        ctx.beginPath();
    ...
    Repeater: (x, y, scale = 1, alpha = 1) => {
        ...
        ctx.fillRect(30, -12, 8, 14); // second barrel
        ctx.fillStyle = '#000';
        ctx.beginPath();
        ctx.arc(5, -10, 4, 0, Math.PI * 2);
        ctx.fill();
    ...
    Squash: (x, y, scale = 1, alpha = 1) => {
        ...
        ctx.fillStyle = '#558b2f';
        ctx.beginPath();
        ctx.ellipse(0, 5, 22, 25, 0, 0, Math.PI * 2);
        ctx.fill();
        // angry eyes
        ctx.fillStyle = '#fff';
        ctx.beginPath();
    ...
    Jalapeno: (x, y, scale = 1, alpha = 1) => {
        ...
        ctx.beginPath();
        ctx.ellipse(0, 5, 14, 28, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.moveTo(0, 33);
        ctx.lineTo(15, 20);
        ...
        ctx.fillStyle = '#33691e';
        ctx.beginPath();
        ctx.ellipse(0, -23, 6, 4, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#000';
        ctx.beginPath();
    ...
    FumeShroom: (x, y, scale = 1, alpha = 1) => {
        ...
        ctx.beginPath();
        ctx.ellipse(0, 15, 16, 12, 0, 0, Math.PI * 2);
        ctx.fill();
    ...
    SplitPea: (x, y, scale = 1, alpha = 1) => {
        ...
        ctx.fill();
        ctx.fillRect(15, -12, 12, 14); // right head
        ctx.beginPath();
        ctx.arc(-10, 0, 16, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillRect(-26, -8, 16, 16); // left head
...
class Cell {
    constructor(x, y) { this.x = x; this.y = y; this.width = cellSize; this.height = cellSize; }
    draw() {
...
function drawUI() {
    ...
    uiGrad.addColorStop(0, '#4e342e');
    uiGrad.addColorStop(1, '#3e2723');
    ctx.fillStyle = uiGrad;
    ctx.fillRect(0, 0, canvas.width, gridOffsetY);
    ctx.fillStyle = '#212121';
    ctx.fillRect(0, gridOffsetY - 4, canvas.width, 4);
    // 阳光计分板
    ctx.fillStyle = '#271915';
    ctx.beginPath();
    ctx.roundRect(15, 15, 100, 70, 10);
    ctx.fill();
    ctx.beginPath();
    ...
    ctx.fillText(sun.toString(), 85, 58);
    // 巨大波次提示 UI
    if (hugeWaveTimer > 0) {
    ...
    for (let i = 0; i < cards.length; i++) {
        ...
        ctx.fillStyle = canAfford ? '#fff' : '#9e9e9e';
    ...
    ctx.beginPath();
    ctx.roundRect(shovelX, shovelY, 60, 70, 8);
    ctx.fill();
    if (selectedCard.isShovel) {
        ctx.strokeStyle = '#aeea00';
        ctx.lineWidth = 3;
        ctx.strokeRect(shovelX - 1, shovelY - 1, 62, 72);
    }
    ctx.save();
    ctx.translate(shovelX + 30, shovelY + 35);
    ...
    ctx.beginPath();
    ctx.roundRect(menuBtnX, menuBtnY, menuBtnW, menuBtnH, 8);
    ctx.fill();
    ...
    // 在进度条右侧画一个小旗子作为终点标志
    ctx.save();
    ctx.translate(barX + barW - 5, barY - 5);
    ctx.fillStyle = '#795548';
    ctx.fillRect(0, 0, 3, 24); // 旗杆
    ctx.fillStyle = '#d32f2f';
    ctx.beginPath();
    ctx.moveTo(3, 0);
    ctx.lineTo(15, 6);
...
class Sun {
    constructor(x, y, targetY, value = 25) {
        this.x = x;
        this.y = y;
        this.targetY = targetY;
        this.value = value;
        this.radius = value === 15 ? 16 : 22;
        this.markedForDeletion = false;
        this.lifeTimer = 0;
    ...
    update() {
        ...
        if (this.y < this.targetY)
            ...
            this.lifeTimer++;
            if (this.lifeTimer > 800)
                this.markedForDeletion = true; // 存活更久
        }
    ...
    draw() {
        ...
        for (let i = 0; i < 8; i++) {
            ctx.beginPath();
            ctx.moveTo(0, 0);
            ctx.lineTo(Math.cos(i * Math.PI / 4) * 28, Math.sin(i * Math.PI / 4) * 28);
...
class Bullet {
    constructor(x, y, vx, vy, type = 'normal', isThreepeater = false) {
        this.x = x;
        this.y = y;
        this.vx = vx;
        this.vy = vy;
...
class Explosion {
    constructor(x, y, type = 'normal') {
        ...
        this.y = y;
        this.radius = 10;
        this.maxRadius = cellSize * 1.5;
        this.life = 30;
        this.markedForDeletion = false;
        this.type = type;
        this.isSquash = type === 'squash';
    ...
    draw() {
        ...
        if (this.isFume) {
            ...
            ctx.beginPath();
            ctx.arc(0, 0, this.radius, 0, Math.PI * 2);
            ctx.fillStyle = this.type === 'cherry' ? '#d32f2f' : '#ff5722';
            ctx.fill();
            ctx.beginPath();
...
class Lawnmower {
    constructor(row) {
        this.row = row;
        this.x = -15;
        this.y = gridOffsetY + row * cellSize;
        this.width = 60;
        this.height = 60;
        this.isActive = false;
        this.markedForDeletion = false;
    ...
    draw() {
        ...
        if (this.isActive) {
            ...
            ctx.beginPath();
            ctx.arc(-30 - Math.random() * 15, -5, Math.random() * 6 + 2, 0, Math.PI * 2);
            ctx.fill();
        }
        ctx.restore();
    }
...
class Plant {
    constructor(x, y, type) {
        this.x = x;
        this.y = y;
        this.width = cellSize - 20;
        this.height = cellSize - 20;
        this.type = type;
        this.timer = 0;
        this.markedForDeletion = false;
        const stats = {
    ...
    update() {
        ...
        if (this.type === 'Peashooter' || this.type === 'SnowPea' || this.type === 'Repeater') {
            ...
            if (this.timer > 600)
                this.isArmed = true; // 10秒后激活 (假设60fps)
            if (this.isArmed) {
            ...
            if (this.timer % 100 === 0 && zombies.some(z => z.y === this.y && z.x > this.x && z.x - this.x < cellSize * 4)) {
                ...
                if (hitAny)
                    playSound('hit');
                for (let i = 0; i < 3; i++)
                    explosions.push(new Explosion(cx + cellSize + Math.random() * cellSize * 2, cy + (Math.random() - 0.5) * 20, 'fume'));
            }
        }
            ...
            if (this.timer > 30) {
                ...
                zombies.forEach(z => { z.slowTimer = 600; z.hp -= 10; if (z.hp <= 0) {
                    z.hp = 0;
                    z.markedForDeletion = true;
                    score++;
                } });
...
class Zombie {
    ...
    draw() {
        ...
        // 身体 (被减速时泛蓝)
        ctx.fillStyle = this.slowTimer > 0 ? '#546e7a' : '#607d8b';
        ctx.fillRect(-15, -10, 30, 50);
        ctx.fillStyle = this.slowTimer > 0 ? '#78909c' : '#78909c';
        ctx.fillRect(-25, this.isEating ? Math.sin(frame * 0.5) * 5 : 0, 20, 10);
        // 头
        ctx.fillStyle = this.slowTimer > 0 ? '#b2ebf2' : '#9e9e9e';
        ctx.beginPath();
        ctx.arc(0, -25 + wobble, 20, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#fff';
        ctx.beginPath();
        ctx.arc(-8, -30 + wobble, 5, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(8, -30 + wobble, 5, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#000';
        ctx.beginPath();
        ctx.arc(-9, -30 + wobble, 2, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(7, -30 + wobble, 2, 0, Math.PI * 2);
        ctx.fill();
        ...
        if (this.typeCode === 2 && this.hp > 150) { // 路障
            ...
            ctx.beginPath();
            ctx.moveTo(0, -65 + wobble);
            ctx.lineTo(-20, -35 + wobble);
            ctx.lineTo(20, -35 + wobble);
            ctx.fill();
            ctx.fillStyle = '#fff';
            ctx.fillRect(-12, -45 + wobble, 24, 5);
        }
        else if (this.typeCode === 3 && this.hp > 150) { // 铁桶
            ctx.fillStyle = '#90a4ae';
            ctx.beginPath();
            ctx.moveTo(-15, -60 + wobble);
            ctx.lineTo(15, -60 + wobble);
            ctx.lineTo(20, -35 + wobble);
            ctx.lineTo(-20, -35 + wobble);
            ctx.fill();
            ctx.fillStyle = '#b0bec5';
            ctx.fillRect(-20, -35 + wobble, 40, 5);
            ctx.fillStyle = '#d32f2f';
            ctx.fillRect(0, -50 + wobble, 10, 10); // 假装有个红标
        }
        // 铁门盾牌
        if (this.typeCode === 4 && this.shieldHp > 0) {
            ...
            ctx.beginPath();
            ctx.roundRect(-30, -50 + wobble, 15, 80, 4);
            ctx.fill();
            ...
            for (let i = 1; i <= 7; i++) {
                ctx.beginPath();
                ctx.moveTo(-30, -50 + wobble + i * 10);
                ctx.lineTo(-15, -50 + wobble + i * 10);
                ctx.stroke();
            }
            for (let i = 1; i <= 2; i++) {
                ctx.beginPath();
                ctx.moveTo(-30 + i * 5, -50 + wobble);
                ctx.lineTo(-30 + i * 5, 30 + wobble);
                ctx.stroke();
            }
            ctx.restore();
        }
...
function handleGameObjects() {
    // 系统状态更新 (CD)
    cards.forEach(c => { if (c.currentCd > 0)
        c.currentCd--; });
    if (hugeWaveTimer > 0)
        hugeWaveTimer--;
    // 剧情生成器：打完一波等5秒出下一波
    ...
    if (zombiesToSpawn > 0 && !waveCompleted) {
        ...
        if (spawnDelayTimer <= 0) {
            ...
            if (currentWave > 5)
                type = Math.random() < 0.5 ? 2 : (Math.random() < 0.5 ? 3 : 4);
            if (currentWave > 10)
                type = Math.random() < 0.3 ? 2 : (Math.random() < 0.5 ? 3 : 4);
    ...
    for (let i = 0; i < zombies.length; i++) {
        ...
        if (z.markedForDeletion)
            continue; // 先行过滤：不要让尸体参与后续判定
        // 僵尸吃植物
        for (let j = 0; j < plants.length; j++) {
            const p = plants[j];
            if (p.type === 'Spikeweed')
                continue; // 僵尸直接踩过地刺，不吃
            if (z.x < p.x + p.width && z.x + z.width > p.x && z.y === p.y && p.type !== 'CherryBomb' && p.type !== 'Jalapeno' && !(p.type === 'PotatoMine' && p.isArmed)) {
                ...
                if (p.type === 'Garlic') {
                    ...
                    if (newY < gridOffsetY)
                        newY = z.y + cellSize;
                    else if (newY >= gridOffsetY + gridRows * cellSize)
                        newY = z.y - cellSize;
                    z.y = newY;
        ...
        // 小推车秒杀
        if (z.markedForDeletion)
            continue; // 如果已经被上一环（比如子弹）击杀，跳过防止被推车鞭尸双倍计分
...
function drawMainMenuUI() {
    ...
    const btnW = baseBtnW * scale;
    const btnH = baseBtnH * scale;
    ctx.save();
    ctx.translate(centerX, centerY);
    ctx.shadowColor = 'rgba(0,0,0,0.4)';
    ctx.shadowBlur = isHover ? 20 : 10;
    ctx.shadowOffsetY = isHover ? 10 : 5;
    ctx.fillStyle = isHover ? '#aeea00' : '#64dd17';
    ctx.beginPath();
    ctx.roundRect(-btnW / 2, -btnH / 2, btnW, btnH, 35 * scale);
    ctx.fill();
    ctx.shadowColor = 'transparent';
    ctx.fillStyle = 'rgba(255, 255, 255, 0.3)';
    ctx.beginPath();
    ctx.roundRect(-btnW / 2 + 4, -btnH / 2 + 4, btnW - 8, btnH / 2 - 4, 25 * scale);
    ctx.fill();
    ctx.fillStyle = isHover ? '#33691e' : '#1b5e20';
...
function drawSeedSelectionUI() {
    ...
    ctx.fill();
    ctx.fillStyle = canStart ? '#1b5e20' : '#424242';
...
function drawPauseMenuUI() {
    ...
    ctx.fillStyle = '#e53935';
    ctx.beginPath();
    ctx.roundRect(minusBtn.x, minusBtn.y, minusBtn.w, minusBtn.h, 8);
    ctx.fill();
    ctx.fillStyle = '#fff';
    ctx.fillText('-', minusBtn.x + 25, minusBtn.y + 28);
    ...
    ctx.beginPath();
    ctx.roundRect(plusBtn.x, plusBtn.y, plusBtn.w, plusBtn.h, 8);
    ctx.fill();
    ctx.fillStyle = '#fff';
    ctx.fillText('+', plusBtn.x + 25, plusBtn.y + 28);
    ...
    ctx.fillStyle = '#fff';
    ctx.font = 'bold 24px Inter';
    ctx.fillText('继续游戏', canvas.width / 2, resumeBtn.y + 34);
    ...
    ctx.fillStyle = '#fff';
    ctx.font = 'bold 24px Inter';
    ctx.fillText('返回菜单', canvas.width / 2, menuBtn.y + 34);
...
function drawEndGameUI() {
    ...
    ctx.fillStyle = '#4caf50';
    ctx.beginPath();
    ctx.roundRect(btn1X, btn1Y, btnW, btnH, 25);
    ctx.fill();
    ctx.fillStyle = 'white';
    ctx.font = 'bold 22px Inter';
    ctx.fillText('主菜单', btn1X + btnW / 2, btn1Y + 33);
